...
create index if not exists idx_case_records_district    on case_records(district_id);
create index if not exists idx_case_records_subcategory on case_records(subcategory_id);
...
create index if not exists idx_user_permissions_subcategory on user_permissions(subcategory_id);
...
-- ===========================================================================
-- Insert sample TB, Ibu Hamil, Stunting data for each district
do $$
declare
  d record;
  tb_sub uuid;
...
-- ===========================================================================
-- Function: verify_login(p_email, p_password)
-- Returns user record if email+password match, else empty result.
-- Uses pgcrypto's crypt() for bcrypt-style password verification.
...
-- ===========================================================================
-- Function: hash_password(p_password)
-- Returns bcrypt hash using pgcrypto's gen_salt + crypt.
-- Dipakai saat membuat/update user dengan password baru.
...
-- ===========================================================================
-- Strategy:
--   - districts, categories, subcategories: PUBLIC READ (anyone can view)
--   - case_records: PUBLIC READ (anyone can view the public map data)
--   - users: ONLY super_admin can READ/WRITE
--   - user_permissions: ONLY super_admin can READ/WRITE
--   - settings: PUBLIC READ (so client can show site_name etc.)
--       BUT groq_api_key is hidden by a view that masks it for non-super_admin
--   - chat_history: user can READ/WRITE only their own; anon cannot read
-- ===========================================================================
...
-- Helper: get current user id from JWT (we'll store user.id in user_metadata)
...
-- Anyone (even anon) can read districts, categories, subcategories, case_records, settings
...
drop policy if exists "public read case_records"  on case_records;
...
-- Any logged-in user (with permission) can insert/update/delete case_records
-- We'll do permission check on the client side AND via RLS using user_id from JWT
drop policy if exists "auth insert case_records" on case_records;
drop policy if exists "auth update case_records" on case_records;
drop policy if exists "auth delete case_records" on case_records;
...
-- Any logged-in user can insert/update/delete (super_admin check on client side)
drop policy if exists "auth write districts"     on districts;
...
-- Only super_admin can read/write users table
-- (since we don't have JWT role-based RLS easily, we'll allow authenticated users
--  to read/write but client will only show this UI to super_admin.
--  For strict security, use Edge Functions instead — see README.md)
...
drop policy if exists "auth read user_permissions"  on user_permissions;
drop policy if exists "auth write user_permissions" on user_permissions;
...
-- User can read/write only their own chat history
drop policy if exists "auth insert chat_history" on chat_history;
drop policy if exists "auth read chat_history"   on chat_history;
...
grant select, insert, update, delete on all sequences in schema public to anon, authenticated;
alter default privileges in schema public grant select, insert, update, delete on tables to anon, authenticated;
alter default privileges in schema public grant select, insert, update, delete on sequences to anon, authenticated;
